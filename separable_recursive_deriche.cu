...
#include "separable_recursive_deriche.cuh"
...
static void gaussianFilterRGBA(uint32_t *d_src, uint32_t *d_dst,
  ...
  const float b1 = -2.0f * ema;

  const float b2 = ema2;

  const float k =
      (1.0f - ema) * (1.0f - ema) / (1.0f + 2.0f * alpha * ema - ema2);

  const float a0 = k;
  const float a1 = k * (alpha - 1.0f) * ema;
  const float a2 = k * (alpha + 1.0f) * ema;
  const float a3 = -k * ema2;

  const float coefp = (a0 + a1) / (1.0f + b1 + b2);

  const float coefn = (a2 + a3) / (1.0f + b1 + b2);

  int threads = 256;

  d_gaussian_rgba<<<iDivUp(width, threads), threads>>>(
      (uint *)d_src, (uint *)d_temp, width, height, a0, a1, a2, a3, b1, b2,
      coefp, coefn);
  ...
  d_gaussian_rgba<<<iDivUp(height, threads), threads>>>(
      (uint *)d_dst, (uint *)d_temp, height, width, a0, a1, a2, a3, b1, b2,
      coefp, coefn);
